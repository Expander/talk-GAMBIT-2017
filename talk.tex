...
\begin{frame}{What is FlexibleSUSY?}
  \begin{center}
    \begin{tikzpicture}
    \node[block] (EXPR) {\textbf{analytic expressions}\\ (\texttt{Mathematica})\\ $\beta_g$, $M_h$, $\Sigma_h$, \ldots};
...
\begin{frame}{Features}
  For all models:
  \begin{itemize}
  \item \MSbar/\DRbar scheme
  \item 2-loop running
  \item 1-loop threshold corrections
  \item 1-loop pole masses
  \item 1-loop $(g-2)_\mu$ + 2-loop leading log QED
  \item 1-loop EDMs
  \end{itemize}
  For the MSSM:
  \begin{itemize}
  \item 3-loop running
  \item 2-loop SQCD threshold corrections to $y_{t,b}$, $\alpha_s$
  \item 2-loop corrections to $M_{h,H,A}$
  \end{itemize}
  For the SM:
  \begin{itemize}
  \item 3-loop running
  \item 3-loop QCD threshold corrections to $y_{t,b}$, $\alpha_s$
