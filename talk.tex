...
\begin{frame}{Hands-on example: High-scale MSSM (HSSUSY)}
  ...
  \begin{tikzpicture}
    \path[arrow] (0,0) -- (0,5) node[left]{$Q$};
    \draw[thick,darkgreen] (0,4) node[left]{$M_S$} -- node[above = 0.5cm,black]{MSSM} (3,4) node[right,black]{fix $\lambda$};
    \draw[thick,dashed,darkgreen] (0,1.5) node[left]{$M_t$} -- (3,1.5) node[right,black]{fix $\mu^2$};
    \draw[thick,darkgreen] (0,1) node[left]{$M_Z$} -- node[above = 1.2cm,black]{SM} node[below = 0.5cm,black]{SM(5)} (3,1) node[right,black]{fix $g_i, y_f, v$};
  \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{Step 2: Define model information}
  ...
  \usebox{\listbox}
  \\[1em]
  \SARAH's \texttt{SM} model defines the parameters:
  \begin{center}
...
\begin{frame}{Step 6: Define high-scale boundary condition}
  Fix $\lambda$ by matching to the MSSM the scale $Q$ = \lstinline{MSUSY}:
  \begin{align*}
...
UseYukawa3LoopQCD = True; (* 3L corr. to yt, yb *)
...
\begin{frame}{Step 7: Enable SM-specific higher order corrections}
  Higher-order corrections from the literature \mycite{1407.4336, hep-ph/0004189}:
  \begin{align*}
