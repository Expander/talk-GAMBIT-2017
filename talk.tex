...
\begin{frame}{Example: CMSSM}
  ...
  \begin{tikzpicture}
    ...
    \draw[thick,dashed] (0,1.5) node[left]{$M_S$} -- (3,1.5) node[right]{fix $|\mu|, B\mu$};
    \draw[thick] (0,1) node[left]{$M_Z$} -- node[above = 1.2cm]{MSSM} node[below = 0.5cm]{SM(5)} (3,1) node[right]{fix $g_i, y_f, v_i$};
  \end{tikzpicture}
  \end{center}
\end{frame}

\begin{frame}{Example: 2HDM-II}
  \begin{center}
  \begin{tikzpicture}
    \path[arrow] (0,0) -- (0,5) node[left]{$Q$};
    \draw[thick] (0,4) node[left]{$M_S$} -- node[above = 0.5cm]{MSSM} (3,4) node[right]{fix $\lambda_i$, $m_{12}^2$};
    \draw[thick,dashed] (0,1.5) node[left]{$M_t$} -- (3,1.5) node[right]{fix $m_1^2$, $m_2^2$};
    \draw[thick] (0,1) node[left]{$M_Z$} -- node[above = 1.2cm]{2HDM-II} node[below = 0.5cm]{SM(5)} (3,1) node[right]{fix $g_i, y_f, v, \tan\beta$};
  \end{tikzpicture}
  \end{center}
\end{frame}
